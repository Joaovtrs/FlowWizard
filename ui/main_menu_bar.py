...
desktop = os.path.join(
    os.path.join(os.environ["USERPROFILE"]), "OneDrive\\Área de Trabalho"
)
...
class MainMenuBar(QMenuBar):
    ...
    def func_open_file(self):
        ...
        if resp == QMessageBox.Yes:
            path = QFileDialog.getOpenFileName(
                parent=self, caption="Abrir", filter="*.fww", dir=desktop
            )
    ...
    def func_save_as_file(self):
        path = QFileDialog.getSaveFileName(
            parent=self, caption="Salvar como", filter="*.fww", dir=desktop
        )
